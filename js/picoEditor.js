/*  PICO Search App
    ...
    v1.3b-dev -- added a trigger that looks for a secret phrase in the search dialog an automatically fires up the pico widget
    v1.3c-dev -- added data element for xboxanchor title value
    v1.3d-dev -- moved help.html ref to widgets.ebscohost.com (255)
*/
...
function picoEditor() {
  ...
  if (jQuery("body").data("picoEditor") == 1) {
    ...
    jQuery("body").attr("data-picoEditor", "1");
  }
  ...
  (pops.label = pops.label || "PICO Search"),
    ...
    (pops.css =
      pops.css ||
      "//widgets.ebscohost.com/prod/simplekey/picoEditor/css/picoEditor.css"),
    (pops.title =
      pops.title || "Opens a separate form for entering a PICO expression");

  jQuery("#outerContainer").after(jQuery("<div />", { id: "picoModal" }));

  // create and assemble the picoBox
  var thePicoBox = jQuery("<div />", {
    class: "picobox",
    id: "picoBox",
    style: "display:none;",
  })
    .append(
      jQuery("<div />", { id: "close-picobox" }).append(
        jQuery("<span />", {
          title: "click to close the query editor textarea",
        })
      )
    )
    .append(
      jQuery("<div />", { id: "picoForm" }).append(
        jQuery("<h2 />").text(pops.header)
      )
    )
  ...
  jQuery("#aspnetForm").after(
    jQuery(thePicoBox).css({
      top: $fieldsetOffsetTop,
      left: $fieldsetOffsetleft,
    })
  );

  // place the picobox search link inline w/ other search options; place picoBox after the SearchTerm1 box
  jQuery("#" + pops.after).after(
    '<li id="picoBoxItem" class="find-field-link"><a href="#" id="picoBoxAnchor" alt="Do a PICO search in EDS">' +
      pops.label +
      "</a></li>"
  );
  ...
  jQuery("#picoBoxAnchor").attr("title", pops.title);
  ...
  // Add CSS
  jQuery("head").append(
    '<link rel="stylesheet" type="text/css" href="' + pops.css + '">'
  );
  ...
  jQuery("#picoSubmit").on("click", function (e) {
    ...
    if (
      jQuery("#picoPopulation").val() == "" ||
      jQuery("#picoIntervention").val() == ""
    ) {
      jQuery("#picoMessage").html(
        '<p style="color: #f44;">Population and Intervention fields are required</p>'
      );
    } else {
      // proceed w/ the search
      var edsSearchTerms = {},
        topics;
      var picoInputs = jQuery(
...
function makeEdsSearchExp(termsObj) {
  ...
  if (termsObj.comparison != "") {
    searchExp +=
      "(" + termsObj.intervention + " OR " + termsObj.comparison + ")"; // (Population) AND (Intervention OR Comparison)
  } else {
...
function getHelpText(topic) {
  switch (topic) {
    case "population":
      //
      return (
        "<h5>Patient problem or Population</h5>" +
...
function openPopup(name, filename, qdata) {
  ...
  var theName = name;
  var theSource =
    "https://widgets.ebscohost.com/prod/simplekey/picoEditor/docs/" +
    filename +
    qData;
  ...
  thePopup = window.open(
    theSource,
    theName,
    "height=680,width=580,top=100,left=100,resizable=yes,scrollbars=yes"
  );
  if (window.focus) {
    thePopup.focus();
  }
...
// comments that are irrelevant and irreproducible
